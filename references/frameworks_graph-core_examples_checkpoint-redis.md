---
title: "checkpoint-redis"
url: "/docs/frameworks/graph-core/examples/checkpoint-redis"
---
...
初始化 RedisSaver[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/CheckpointRedisExample.java)
```java
...
完整示例[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/CheckpointRedisExample.java)
```java
...
Spring Boot 配置类[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/CheckpointRedisExample.java)
```java
...
恢复工作流[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/CheckpointRedisExample.java)
```java
import com.alibaba.cloud.ai.graph.CompiledGraph;
import com.alibaba.cloud.ai.graph.RunnableConfig;
...
查询历史状态[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/CheckpointRedisExample.java)
```java
import com.alibaba.cloud.ai.graph.CompiledGraph;
import com.alibaba.cloud.ai.graph.RunnableConfig;
import com.alibaba.cloud.ai.graph.state.StateSnapshot;
...
从特定检查点恢复[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/CheckpointRedisExample.java)
```java
import com.alibaba.cloud.ai.graph.CompiledGraph;
import com.alibaba.cloud.ai.graph.RunnableConfig;
...
- **连接池**：Redisson 自动管理连接池，优化 Redis 连接性能。

- **内存优化**：合理配置 Redis 内存限制，避免内存溢出。

- **TTL 设置**：为检查点数据设置合理的过期时间，自动清理旧数据。

- **批量操作**：利用 Redis Pipeline 减少网络往返次数。

- **持久化配置**：根据需求配置 RDB 或 AOF 持久化策略。
...
测试连接[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/CheckpointRedisExample.java)
```java
// 测试 Redis 连接
try {
  Config config = new Config();
...
- **唯一的线程 ID**：为每个独立的工作流实例使用唯一的线程 ID。

- **定期备份**：配置 Redis 持久化（RDB 或 AOF）以防止数据丢失。

- **监控**：监控 Redis 内存使用、连接数和性能指标。

- **TTL 策略**：为检查点数据设置合理的过期时间，自动清理旧数据。

- **安全性**：使用 Redis 密码认证，限制网络访问，启用 TLS 加密。

- **高可用**：生产环境建议使用 Redis Sentinel 或 Cluster 模式。

- **内存管理**：合理配置 Redis 最大内存和淘汰策略。
