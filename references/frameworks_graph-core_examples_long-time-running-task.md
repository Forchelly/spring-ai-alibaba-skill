---
title: "long-time-running-task"
url: "/docs/frameworks/graph-core/examples/long-time-running-task"
---
...
提示如果您在使用 Spring AI Alibaba Graph 时配置了检查点器（checkpointer），则已经启用了持久化执行。您可以在任何时候暂停和恢复工作流，即使在中断或失败之后也是如此。

为了充分利用持久化执行，请确保您的工作流设计为[确定性和一致性重放](#%E7%A1%AE%E5%AE%9A%E6%80%A7%E5%92%8C%E4%B8%80%E8%87%B4%E6%80%A7%E9%87%8D%E6%94%BE)，并将任何副作用或非确定性操作包装在节点中。
...
- 
将任何非确定性操作（例如，随机数生成）或具有副作用的操作（例如，文件写入、API 调用）包装在节点中，以确保在恢复工作流时，这些操作不会在特定运行中重复，而是从持久化层检索它们的结果。有关更多信息，请参阅[确定性和一致性重放](#%E7%A1%AE%E5%AE%9A%E6%80%A7%E5%92%8C%E4%B8%80%E8%87%B4%E6%80%A7%E9%87%8D%E6%94%BE)。
...
编译时配置持久化策略[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/LongTimeRunningTaskExample.java)
```java
...
执行时配置[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/LongTimeRunningTaskExample.java)
```java
import com.alibaba.cloud.ai.graph.CompiledGraph;
...
原始版本[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/LongTimeRunningTaskExample.java)
```java
...
改进版本[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/LongTimeRunningTaskExample.java)
```java
import com.alibaba.cloud.ai.graph.StateGraph;
import static com.alibaba.cloud.ai.graph.action.AsyncNodeAction.node_async;
...
- **暂停和恢复工作流**：使用中断机制在特定点暂停工作流，并使用更新的状态恢复它。有关更多详细信息，请参阅[人在回路中文档](/docs/frameworks/graph-core/examples/human-in-the-loop)。

- **从失败中恢复**：在异常后（例如，LLM 提供商中断）从最后一个成功的检查点自动恢复工作流。这涉及使用相同的线程标识符执行工作流。

### 从错误中恢复示例

从错误中恢复示例[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/LongTimeRunningTaskExample.java)
```java
import com.alibaba.cloud.ai.graph.RunnableConfig;
import com.alibaba.cloud.ai.graph.CompiledGraph;
...
- **幂等性设计**：确保节点操作是幂等的，多次执行产生相同结果。

- **状态管理**：合理设计状态结构，避免存储不必要的大对象。

- **错误处理**：在节点中实现适当的错误处理和重试逻辑。

- **监控和日志**：添加适当的日志记录以跟踪工作流执行进度。

- **测试**：彻底测试暂停和恢复场景，确保工作流行为正确。

## 示例：长时间运行的数据处理任务

长时间运行的数据处理任务示例[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/LongTimeRunningTaskExample.java)
```java
import com.alibaba.cloud.ai.graph.StateGraph;
import com.alibaba.cloud.ai.graph.CompiledGraph;
import static com.alibaba.cloud.ai.graph.action.AsyncNodeAction.node_async;
