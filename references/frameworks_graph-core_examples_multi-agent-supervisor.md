...
url: "/docs/frameworks/graph-core/examples/multi-agent-supervisor"
---
...
多智能体监督者模式实现[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/MultiAgentSupervisorExample.java)
```java
...
执行多智能体监督者图[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/MultiAgentSupervisorExample.java)
```java
...
- **监督者路由**：监督者智能体根据用户请求决定将任务路由到哪个工作智能体

- **条件边**：使用条件边实现动态路由

- **工具集成**：工作智能体可以使用工具（如搜索工具、代码执行工具）来完成任务

- **状态管理**：使用 `messages` 和 `next` 键来管理对话状态和路由决策

## 最佳实践

- **清晰的系统提示**：为监督者智能体提供清晰的系统提示，确保它能正确路由任务

- **工具设计**：为每个工作智能体设计合适的工具，使其能够高效完成任务

- **错误处理**：在节点中添加适当的错误处理逻辑

- **状态规范化**：实现路由结果的规范化逻辑，确保路由决策的一致性
