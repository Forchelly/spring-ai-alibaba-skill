...
url: "/docs/frameworks/graph-core/examples/parallel-streaming"
---
...
并行节点流式输出示例[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/ParallelStreamingExample.java)
```java
...
单个节点的流式输出示例[查看完整代码](https://github.com/alibaba/spring-ai-alibaba/tree/main/examples/documentation/src/main/java/com/alibaba/cloud/ai/examples/documentation/graph/examples/ParallelStreamingExample.java)
```java
/**
* 示例 2: 单个节点的流式输出
*
...
- **直接使用 Flux**：节点可以直接返回 `Flux<T>` 类型的流式数据，系统会自动识别并处理

- **节点 ID 保持**：每个并行节点的流式输出会保持各自的节点 ID，便于区分不同节点的输出

- **独立流式处理**：每个并行节点可以独立产生和处理流式数据，互不干扰

- **结果统计**：可以统计每个节点产生的流式输出数量，便于监控和调试
...
- **直接返回 Flux**：在节点中直接返回 `Flux<T>` 类型的数据，系统会自动识别并处理为流式输出

- **节点 ID 命名**：为每个并行节点使用清晰、有意义的节点 ID，便于区分不同节点的输出

- **延迟控制**：使用 `delayElements()` 合理设置流式数据的延迟，避免过快或过慢

- **错误处理**：在流式处理中添加适当的错误处理逻辑，使用 `doOnError()` 处理异常

- **结果统计**：使用统计机制跟踪流式输出的进度和数量，便于监控和调试

- **线程信息**：可以在节点中添加线程信息输出，便于理解并行执行的机制
